...
public class D_switch {
    ...
    public void testSimpleSwitchStatement(){
        ...
        *  */
        ...
        System.out.print("첫 번쨰 정수를 입력해 주세요 : ");
        int first = sc.nextInt();
        System.out.print("두 번째 정수를 입력해 주세요 : ");
        int second = sc.nextInt();
        System.out.println("연산 기호 입력(+,-,*,/,%");
        char op =sc.next().charAt(0);    //중요!! 문자 char 일 경우, 이렇게 입력!! // (0)은 0번쨰 문자 반환받기

        /* 1. 연산의 결과를 저장할 변수 생성 */
        int result = 0;

        /* (op) = 입력한 연산기호 문자(사칙~)에 따라 연산의 결과가 달라짐 */
        switch (op) {
            case '+' :
                result = first + second;
                break;           // (분기문)한번 멈춰준다.
    ...
    public void testSwitchVendingMachine(){
        ...
        /* 필기. 2. switch 문을 이용한 간단한 자판기 만들어보기 */
        ...
        Scanner sc = new Scanner(System.in);
        String selectDrink = sc.nextLine();      // 콘솔에 선택(입력)한 음료가 담기게 됨

        /* 연습. 원하는 음료에 맞는 가격을 저장할 변수 */
        int price = 0;
        //int price;  이렇게 선언만 해도 가능하다.
